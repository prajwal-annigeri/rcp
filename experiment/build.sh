...
CODE_FOLDER_NAME="${2%.*}"
YCSB_ZIP_FILE_NAME=$3
YCSB_FOLDER_NAME="${3%.*}"

if [[ $# -ne 3 ]]; then
  echo "Usage: $0 <public_ip> <code_zip_file_name> <ycsb_zip_file_name>"
  exit 1
fi

# Copy zip to EC2
echo "Copying zip $CODE_ZIP_FILE_NAME and $YCSB_ZIP_FILE_NAME"
scp -i "$KEY" -o StrictHostKeyChecking=no "$CODE_ZIP_FILE_NAME" $USER@$IP:~
scp -i "$KEY" -o StrictHostKeyChecking=no "$YCSB_ZIP_FILE_NAME" $USER@$IP:~
...
ssh -i "$KEY" -o StrictHostKeyChecking=no $USER@$IP << EOF
  if ! command -v go &> /dev/null; then
    ...
    if [ -f /etc/debian_version ]; then
      sudo apt update && sudo apt install -y golang unzip
    else
      sudo yum install -y golang unzip
    fi
  fi
  unzip -o ~/$CODE_ZIP_FILE_NAME
  unzip -o ~/$YCSB_ZIP_FILE_NAME
  cd ~/$CODE_FOLDER_NAME
